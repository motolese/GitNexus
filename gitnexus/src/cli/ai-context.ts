...
function generateGitNexusContent(
  projectName: string,
  stats: RepoStats,
  _generatedSkills?: GeneratedSkillInfo[],
  groupNames?: string[],
): string {
  // Slim block (bitcoinize fork): one-line mention only. Detail lives in
  // .claude/skills/gitnexus/* — load on demand, not in every session prefix.
  const groupsSuffix =
    groupNames && groupNames.length > 0
      ? ` Cross-repo groups: ${groupNames.join(', ')}.`
      : '';

  return `${GITNEXUS_START_MARKER}
GitNexus indexes \`${projectName}\` (${stats.nodes || 0} symbols / ${stats.edges || 0} relationships / ${stats.processes || 0} flows). Tools: \`mcp__datta__gitnexus_*\`. Skills: \`.claude/skills/gitnexus/\`. Refresh: \`gitnexus analyze\`. Detail: \`.claude/skills/gitnexus/gitnexus-guide/SKILL.md\`.${groupsSuffix}
${GITNEXUS_END_MARKER}`;
}
